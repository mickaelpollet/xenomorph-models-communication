...
/**********************************************************************/
	...
	public function connexion(XLdapConnection $LdapConnection) {
		...
		try {
			...
			$ldap_login = ldap_bind($ldap_connexion, $LdapConnection->user(), $LdapConnection->password());
	...
	public function search ($search){
		...
		try {
			...
			if (is_array($search)) { // Si le paramètre $search est un tableau
				...
				// recherche LDAP
				$searching = ldap_search($ldapConnexion, $ldap_base, $ldap_filter);

				// résultat de la recherche
				$info = ldap_get_entries($ldapConnexion, $searching);
				...
				for ($i = 0; $i < $info['count']; $i++) {
					...
					$current_result = new LdapUser();

					$current_result->setCn($info[$i]['cn'][0]);
					$current_result->setSn($info[$i]['sn'][0]);
					$current_result->setUid($info[$i]['uid'][0]);
					$current_result->setDn($search_type);
					$current_result->setdisplayName($info[$i]['displayName'][0]);
					$current_result->setGivenName($info[$i]['givenName'][0]);
					$current_result->setPostalCode($info[$i]['postalcode'][0]);
					$current_result->setL($info[$i]['l'][0]);
					$current_result->setTitle($info[$i]['title'][0]);
					$current_result->setEmployeeNumber($info[$i]['employeeNumber'][0]);
					$current_result->setMail($info[$i]['mail'][0]);
	...
	public function delete(XLdap $XLdap)	{
		...
		try{
			...
			if ($user_check['result'] == 'success' && $user_check['message'] != false) { // si l'utilisateur existe
				...
				$ldap_deleting = ldap_delete($ldapConnexion, $XLdap->dn());
	...
	public function update(XLdap $XLdap)	{
		...
		try {
			...
			if ($user_check['result'] == 'success' && $user_check['message'] != false) {
				...
				// Ajout de l'utilisateur
				$ldap_modifying = ldap_modify($ldapConnexion, $XLdap->dn(), $XLdap->ldapObject());
